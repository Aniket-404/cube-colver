 ...
 * Each cube type has specific constraints regarding facelet counts, color distribution,
 * and solvability requirements that must be enforced.
 */
...
export const VALIDATION_ERRORS = {
    ...
    // Include solvability error types
    ...SOLVABILITY_ERRORS
};
...
export function validate2x2x2(cubeState) {
    ...
    // Step 6: Check solvability (mathematical constraints)
    const solvabilityResult = checkSolvability(cubeState);
    if (!solvabilityResult.isSolvable) {
        solvabilityResult.errors.forEach(solvabilityError => {
            errors.push({
                type: VALIDATION_ERRORS.UNSOLVABLE_STATE,
                message: `2x2x2 cube is mathematically unsolvable: ${solvabilityError.message}`,
                details: solvabilityError
            });
        });
        return {
            isValid: false,
    ...
    return {
        ...
        summary: {
            ...
            solvabilityChecks: solvabilityResult.checks
        }
    };
}
...
export function validate3x3x3(cubeState) {
    ...
    // Step 7: Check solvability (mathematical constraints)
    const solvabilityResult = checkSolvability(cubeState);
    if (!solvabilityResult.isSolvable) {
        solvabilityResult.errors.forEach(solvabilityError => {
            errors.push({
                type: VALIDATION_ERRORS.UNSOLVABLE_STATE,
                message: `3x3x3 cube is mathematically unsolvable: ${solvabilityError.message}`,
                details: solvabilityError
            });
        });
        return {
...
export function validate4x4x4(cubeState) {
    ...
    if (!solvabilityResult.isSolvable) {
        solvabilityResult.errors.forEach(solvabilityError => {
            errors.push({
                type: VALIDATION_ERRORS.UNSOLVABLE_STATE,
                message: `4x4x4 cube is mathematically unsolvable: ${solvabilityError.message}`,
                details: solvabilityError
            });
        });
        return {
...
export function getValidationSummary(cubeState) {
    ...
    switch (cubeState.cubeType) {
        ...
        case '4x4x4':
            return validate4x4x4(cubeState);
        default:
            return {
                isValid: false,
                errors: [{
...
export default {
    ...
    // 4x4x4 specific validation
    validateColorDistribution4x4x4,
    validateCenters4x4x4,
    validate4x4x4,
